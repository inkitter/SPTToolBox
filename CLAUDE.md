...
Two independent, parallel implementations of the same feature (floating damage numbers over an enemy on a hit), each behind its own `Settings` toggle so they can be compared or one disabled without touching the other:

- `Plugin/Utils/HitDamagePopupRenderer.cs` (`Settings.ShowHitDamageNumbers`, on by default) — polls every alive enemy's summed tracked-body-part HP every frame via `HealthController.GetBodyPartHealth` and diffs against the last poll. Simple, but calling into a possibly-just-destroyed player's `HealthController` on the same frame they die is the suspected cause of an intermittent fatal `AccessViolationException` from `il2cpp_runtime_invoke` (native crash, not a catchable managed exception — the `try/catch` around `Tick()` in `SPTMapController` doesn't stop it). Was originally event-driven via `ActiveHealthController.ApplyDamageEvent`, but that event's `DamageInfo` struct parameter isn't blittable, so every subscribe attempt threw inside `DelegateSupport.ConvertDelegate` — repeated failing native interop calls are suspected to have corrupted Il2Cpp runtime state badly enough to cause unrelated crashes elsewhere, which is why this was rewritten to polling instead.
- `Plugin/Utils/BulletHitPopupRenderer.cs` (`Settings.ShowHitDamageNumbersV2`, off by default, experimental) — subscribes directly to each tracked player's `Player.OnDamageReceived` event instead of polling. That delegate's signature (`Action<float damage, EBodyPart, EDamageType, float armorDamage, MaterialType>`) is all plain value types, so `DelegateSupport.ConvertDelegate` handles it fine — unlike `ApplyDamageEvent`, this one was never a repro of the failed-subscribe problem above. The callback itself only enqueues a value-type struct (no further Il2Cpp calls); the actual popup spawn/position lookup happens on the next `Tick()`. Uses the hit `EBodyPart` to place the popup at that specific `BodyPartType` position instead of always defaulting to the head. Not yet confirmed in-game as of 2026-09-21 — the goal is to see whether it also avoids the `AccessViolationException` `HitDamagePopupRenderer` occasionally hits, since it never touches `HealthController` at all.

Subscribing to `Player.OnDamageReceived` needs `player.add_OnDamageReceived(handler)` with an explicitly-typed `System.Action<...>` local, not `player.OnDamageReceived += lambda` — the implicit `System.Action<...> → DamageDelegate` conversion the game's own interop code generates only resolves through a method-argument/explicit-cast context, not a bare `+=` on an inline lambda. Also note this is one of the few places `System.Action<>` (not `Il2CppSystem.Action<>`) is actually correct for an Il2Cpp delegate — see the IL2CPP interop notes below for why that's normally backwards.
...
| `Plugin/Utils/EnemyEspRenderer.cs` | Enemy ESP overlay (box/distance/AI info), also supplies the shared camera/world-to-GUI helpers used by the hit damage popup renderers |
| `Plugin/Utils/HitDamagePopupRenderer.cs` | Hit damage popups, poll-based (default) — see "Hit damage popups" above |
| `Plugin/Utils/BulletHitPopupRenderer.cs` | Hit damage popups, event-based (`OnDamageReceived`, experimental) — see "Hit damage popups" above |
...
- 2026-09-21: added `BulletHitPopupRenderer.cs` as an event-driven (`Player.OnDamageReceived`) alternative to the polling `HitDamagePopupRenderer.cs`, gated by a new `Settings.ShowHitDamageNumbersV2` (off by default) — see "Hit damage popups" above. Motivated by an intermittent `AccessViolationException` reported from `HitDamagePopupRenderer.Tick`'s `HealthController.GetBodyPartHealth` call; not yet confirmed in-game whether the new path avoids it.
...
- Use `Il2CppSystem.Action<>` / `Il2CppSystem.Func<>` (not `System.`) when wiring Il2Cpp delegates (event handlers, callbacks on EFT types) — **except** where the generated Il2Cpp delegate type itself provides an `implicit operator` from a plain `System.Action<...>` (check the interop DLL's decompile for the delegate type, e.g. `Player.DamageDelegate`); there, `System.Action<>` is correct and expected. See "Hit damage popups" above for a real example.
