...
`Plugin/Utils/BulletHitPopupRenderer.cs` (`Settings.ShowHitDamageNumbers`, on by default) subscribes to each tracked player's `Player.OnDamageReceived` event and spawns a floating number at the hit `EBodyPart`'s position. The callback only enqueues a value-type struct; spawn/position lookup happens on the next `Tick()`. That delegate's signature is all plain value types, so `DelegateSupport.ConvertDelegate` handles it fine. It replaced (2026-09-22, deleted) an older poll-based `HitDamagePopupRenderer` that diffed every enemy's `HealthController.GetBodyPartHealth` each frame - suspected cause of an intermittent native `AccessViolationException` when reading a player destroyed that same frame. Don't reintroduce `ActiveHealthController.ApplyDamageEvent`: its `DamageInfo` parameter isn't blittable and every subscribe attempt throws inside `ConvertDelegate`.
...
| `Plugin/Utils/EnemyEspRenderer.cs` | Enemy ESP overlay (box/distance/AI info; second pass through the optic camera into the lens rect while scoped), also supplies the shared camera/world-to-GUI helpers used by the hit damage popup renderers |
| `Plugin/Utils/BulletHitPopupRenderer.cs` | Hit damage popups (`OnDamageReceived` event-based) — see "Hit damage popups" above |
| `Plugin/Config/Settings.cs` | BepInEx `ConfigEntry` bindings |
| `Plugin/DynamicMarkers/MapFeatures/` | Extract, transit, door, power-switch marker providers |
